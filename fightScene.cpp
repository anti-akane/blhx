...
void fightScene::init() {
    ...
    totalempty.push_back(
            new standardEmpty(10000, 100, 100, 2, 5, 90, 1000, QPixmap("://res/liangchan-1.png"), QPoint(700, 200)));
    totalempty.push_back(
            new standardEmpty(10000, 100, 100, 2, 5, 90, 1000, QPixmap("://res/liangchan-1.png"), QPoint(900, 100)));
    totalempty.push_back(
            new humanoidEmpty(20000, 400, 100, 3, 110, 1000, QPixmap("://res/Yukikaze.png"), QPoint(900, 300)));
    totalempty.push_back(
            new standardEmpty(10000, 100, 100, 2, 5, 100, 100, QPixmap("://res/liangchan-1.png"), QPoint(900, 500)));
    totalempty.push_back(
            new standardEmpty(10000, 100, 100, 2, 5, 90, 1000, QPixmap("://res/liangchan-1.png"), QPoint(700, 400)));
    totalempty[0]->setState(1);
    totalempty[1]->setState(2);
    totalempty[2]->setState(2);
    totalempty[3]->setState(2);
    totalempty[4]->setState(1);
         backwarship.push_back(new carrierVessel(8000,300,0,0,100,0,QPixmap("://res/unicorn.png"),QPoint(0,100),700,2,400));
    backwarship.push_back(new battleShip(8000,400,0,0,1000,0,QPixmap(":/res/Richelieu.png"),QPoint(0,300),1000,1));
      backwarship.push_back(new battleShip(8000,200,0,0,100,0,QPixmap("://res/Hutten.png"),QPoint(0,500),500,1));
    //三个技能按钮的初始化
...
void fightScene::frontshoot(frontWarShip*WarShip) {
    if (WarShip->shoot()) {
        QPoint *goal = findEmpty(QPoint(WarShip->getlocation().x(), WarShip->getlocation().y()));
...
void fightScene::torp() {
    ...
    cannonball.push_back(new cannonBall(torp->getparent()->getlocation().x(), torp->getparent()->getlocation().y(),
                                        torp->getparent()->torp_hurt(),10, -1.0 * pi / 30.0, nullptr, ":/res/torp.png",35));
    cannonball.push_back(new cannonBall(torp->getparent()->getlocation().x(), torp->getparent()->getlocation().y(),
                                        torp->getparent()->torp_hurt(),10, pi / 40.0, nullptr, ":/res/torp.png",35));
    cannonball.push_back(new cannonBall(torp->getparent()->getlocation().x(), torp->getparent()->getlocation().y(),
                                        torp->getparent()->torp_hurt(),10, -1.0 * pi / 40.0, nullptr, ":/res/torp.png",35));
...
void fightScene::autoOperate() {
    ...
    QMouseEvent *release = new QMouseEvent(QEvent::MouseButtonRelease, QPoint(0, 0), Qt::LeftButton, Qt::LeftButton,
                                           Qt::NoModifier);
    if (torpedoesButton->getSkillNumber()&&!emptylist.empty())
        QApplication::postEvent(torpedoesButton, release);
    if (planeButton->getSkillNumber()&&!emptylist.empty())
        QApplication::postEvent(planeButton, release);
    if (navalgunButton->getSkillNumber()&&!emptylist.empty())
        QApplication::postEvent(navalgunButton, release);
}
...
void fightScene::emptyshoot() {
    for (auto v: emptylist) {
        if (v->shoot()) {
            ...
            emptycannon.push_back(new cannonBall(b_x, b_y - 5, v->power_hurt() / 10,30,
        ...
        if (check != nullptr) {
            if (check->check_torp()) {
                emptycannon.push_back(new cannonBall(v->getlocation().x(), v->getlocation().y() + v->getheight() / 2,
                                                     v->torp_hurt() / 10, 20,0 + pi, nullptr, ":/res/torp.png",35
                                                     ));
                emptycannon.push_back(new cannonBall(v->getlocation().x(), v->getlocation().y() + v->getheight() / 2,
                                                     v->torp_hurt() / 10,20, pi / 8 + pi, nullptr, ":/res/torp.png",35));
                emptycannon.push_back(new cannonBall(v->getlocation().x(), v->getlocation().y() + v->getheight() / 2,
                                                     v->torp_hurt() / 10,20, -pi / 8 + pi, nullptr, ":/res/torp.png",35));
            }
        }
...
void fightScene::battleShoot()
{
    ...
    else
    {
        ...
        for(auto v:emptylist)
        {
            if(v->getHP()>Max)
            {
                end=new QPoint(v->getlocation().x()+v->getTachie().width()/2,v->getlocation().y()+v->getTachie().height()/2);
                Max=v->getHP();
            }
...
void fightScene::Boundary()
{
    ...
    for (QVector<emptyWarShip *>::iterator empty = emptylist.begin(); empty != emptylist.end();) {
        if ((*empty)->getlocation().x()<=10) {

           backwarship[(*empty)->getlocation().y()/(675.0/n)]->declineHP(1000*(*empty)->getHpRate());
           emptylist.erase(empty);
        } else
            empty++;
    }
            ...
            if((*item)->check())
            {
               ...
               { int tx=500 + qrand() % 600,ty=100+qrand()%450;
                cannonball.push_back(new battleCannon(tx, ty,
                 (*item)->getHurt()/4,1, 0, nullptr, "://res/bombing.png",QPoint(tx+3,ty),120));
        ...
        else
        {
             if((*item)->check())
            {
                 ...
                 cannonball.push_back(new cannonBall((*item)->getX(), (*item)->getY()+10,
                                                      (*item)->getHurt(),15,
                                                     cal_angle(now,findEmpty(*now)), nullptr, ":/res/torp.png",35));
                 cannonball.push_back(new cannonBall((*item)->getX(), (*item)->getY()-10,
                                                      (*item)->getHurt(),15,
                                                     cal_angle(now,findEmpty(*now)), nullptr, ":/res/torp.png",35));
                                aircraft.erase(item);
...
QPoint* fightScene::findEmpty(QPoint start)
{
...
void fightScene::checkEmpty()
{
    if(emptylist.empty())
       { state++;
         for (QVector<emptyWarShip *>::iterator item = totalempty.begin(); item != totalempty.end();) {
        {
                         humanoidEmpty *check = dynamic_cast<humanoidEmpty *>(*item);
                         standardEmpty *another = dynamic_cast<standardEmpty *>(*item);
            if((*item)->getState()==state)
            {
                if(check!=nullptr)
                emptylist.push_back(new humanoidEmpty(*check));
                else
                emptylist.push_back(new standardEmpty(*another));
                totalempty.erase(item);
            }
            else
                item++;
        }
